...
MEarth = named_unit("MEarth", "MEarth", 5.97219e24 * units.kg)
...
def get_mass_and_orbital_separation(
    a, surface_density_factor=1.0, mstar=1 | units.MSun
):
...
def new_planet_distribution(
    x0, x1, target_total_mass, accurancy, max_iterations=1000, mstar=1 | units.MSun
):
    ...
    while not is_hit(total_mass, target_total_mass, accurancy):
        ...
        current_surface_density_factor = (
            min_surface_density_factor + max_surface_density_factor
        ) / 2.0

        mass, pos = new_distribution(
            x0, x1, current_surface_density_factor, mstar=mstar
        )
        total_mass = mass.sum()
...
def new_rotation_matrix_from_euler_angles(phi, theta, chi):
    ...
    return numpy.array(
        [
            [
                cost * cosc,
                -cosp * sinc + sinp * sint * cosc,
                sinp * sinc + cosp * sint * cosc,
            ],
            [
                cost * sinc,
                cosp * cosc + sinp * sint * sinc,
                -sinp * cosc + cosp * sint * sinc,
            ],
...
def rotate(position, velocity, phi, theta, psi):  # theta and phi in radians
    ...
    return (
        numpy.dot(matrix, position.value_in(Runit)) | Runit,
...
def posvel_from_orbital_elements(Mstar, semimajor_axis, eccentricity, kepler, rng=None):
    ...
    kepler.initialize_from_elements(
        Mstar, semimajor_axis, eccentricity, mean_anomaly=mean_anomaly
    )
...
def make_planets(
    ...
    psi = numpy.radians(rng.uniform(0.0, 180.0, 1))[
        0
    ]  # 0 # numpy.radians(90) # numpy.radians(rng.uniform(0, 180, 1))[0]
    com_particle = central_particle.copy()
    for x, t in zip(iter(planet_particles), theta):
        pos, vel = posvel_from_orbital_elements(
            com_particle.mass + x.mass, x.semimajor_axis, x.eccentricity, kepler, rng
        )
        ...
        if False:
            ...
            print(
                "dp:",
                (com_particle.position - two_body.center_of_mass()).as_quantity_in(
                    units.AU
                ),
            )
...
def new_system(
    ...
    m, r, f = new_planet_distribution(
        disk_minimum_radius, disk_maximum_radius, disk_mass, accurancy
    )
...
def new_option_parser():
    ...
    result.add_option(
        ...
        unit=units.AU,
        type="float",
        default=0.3 | units.AU,
        dest="disk_minimum_radius",
        help="minimum radius of the disk [%default]",
    )
    result.add_option(
        "--amax",
        unit=units.AU,
        type="float",
        default=300 | units.AU,
        dest="disk_maximum_radius",
        help="maximum radius of the disk [%default]",
    )
    result.add_option(
        "--Nstars",
        type="int",
    ...
    result.add_option(
        ...
        unit=units.MSun,
        type="float",
        default=0.3 | units.MSun,
        dest="minimum_stellar_mass",
        help="minimum mass of primary star [%default]",
    )
    result.add_option(
        "--Mmax",
        unit=units.MSun,
        type="float",
        default=3.0 | units.MSun,
        dest="maximum_stellar_mass",
        help="maximum mass of primary star [%default]",
    )
    result.add_option(
        "--Mdisk",
        unit=MEarth,
    ...
    result.add_option(
        ...
        type="float",
        default=0.01,
        dest="disk_mass_fraction",
        help="fraction of the mass of the disk [%default]",
    )
    result.add_option(
        "--planet-density",
        unit=units.g / units.cm**3,
        type="float",
        default=3 | units.g / units.cm**3,
        dest="planet_density",
        help="density of the planets  (used for planet radii calculation) [%default]",
    )
    result.add_option(
        "--accuracy",
        type="float",
        default=0.0001,
        dest="accuracy",
        help="how accurate should the disk mass be integrated to[%default]",
    )
...
if __name__ == "__main__":
    ...
    if o.Nstars == 1:
        ...
        star_with_planets = make_planets_oligarch(
            star_mass=o.star_mass,
        ...
        masses = new_kroupa_mass_distribution(
            o.Nstars, mass_min=o.minimum_stellar_mass, mass_max=o.maximum_stellar_mass
        )
